...
| **`/restwalker:teleport`** | "what was I doing in <folder>", "carry over the conversation from my other Mac" | Pulls a recent Claude conversation from another folder or another Mac into this session (`teleport` / `teleport_list` / `teleport_folders`; cross-Mac via Bash). |
